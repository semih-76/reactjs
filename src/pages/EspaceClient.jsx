  ...
  <div className="ec-view-dashboard">
      ...
      <div className="last-order-section">
        ...
        <div className="last-order-card">
          <div className="order-info">
            ...
            <span
              className={`status-badge ${stats.derniereCommande.statut.toLowerCase().replace(/ /g, "-")}`}
            >
              {stats.derniereCommande.statut}
            </span>
            <span className="order-price">
              {stats.derniereCommande.prix.toFixed(2)} €
            </span>
          </div>
          <button
    ...
    <div className="quick-access">
      ...
      <div className="access-grid">
        ...
        <div
          className="access-card"
          onClick={() => setActiveTab("mes-adresses")}
          style={{ cursor: "pointer" }}
        >
          <span className="icon">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
        ...
        <div
          className="access-card"
          onClick={() => setActiveTab("mes-informations")}
          style={{ cursor: "pointer" }}
        >
          <span className="icon">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
        ...
        commandesFiltrees.map((cmd) => (
          <div className="order-item-card" key={cmd.id}>
            <div className="order-card-header">
              ...
              <span
                className={`status-badge ${cmd.statut.toLowerCase().replace(/ /g, "-")}`}
              >
                {cmd.statut}
              </span>
            ...
            <div className="order-actions">
              <button
                ...
                onClick={() => alert(`Détails de la commande ${cmd.id}`)}
              >
                Voir détails
              </button>
              <button
                className="btn-dark-sm"
                onClick={() => alert(`Commande ${cmd.id} ajoutée au panier`)}
              >
...
const MesAdresses = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (editId !== null) {
      setAdresses(
        adresses.map((a) => (a.id === editId ? { ...a, ...formData } : a)),
      );
      setEditId(null);
    } else {
    ...
    <div className="ec-view-addresses">
      ...
      <div className="address-grid">
        {adresses.map((adresse) => (
          <div
            className={`address-card ${adresse.defaut ? "default" : ""}`}
            key={adresse.id}
          >
            ...
            <div className="address-actions">
              <button className="btn-link" onClick={() => handleEdit(adresse)}>
                Modifier
              </button>
              {!adresse.defaut && (
                <button
                  className="btn-link"
                  style={{ color: "red" }}
                  onClick={() => handleSupprimer(adresse.id)}
                >
                  Supprimer
                </button>
              )}
...
const MesInformations = () => {
  ...
  const handleSaveMdp = (e) => {
    e.preventDefault();
    if (!mdp.ancien) {
      setMsgMdp("Veuillez saisir votre ancien mot de passe.");
      return;
    }
    if (mdp.nouveau !== mdp.confirmation) {
      setMsgMdp("Les nouveaux mots de passe ne correspondent pas.");
      return;
    }
    if (mdp.nouveau.length < 8) {
      setMsgMdp("Le mot de passe doit contenir au moins 8 caractères.");
      return;
    }
    ...
    <div className="ec-view-info">
      ...
      <section className="info-section">
        ...
        <form onSubmit={handleSaveInfo}>
          ...
          {msgInfo && (
            <p style={{ color: "green", marginBottom: "8px" }}>{msgInfo}</p>
          )}
      ...
      <section className="info-section">
        ...
        <form onSubmit={handleSaveMdp}>
          ...
          {msgMdp && (
            <p
              style={{
                color: msgMdp.includes("succès") ? "green" : "red",
                marginBottom: "8px",
              }}
            >
              {msgMdp}
      ...
      <section className="info-section danger-zone">
        ...
        <button className="btn-danger-outline" onClick={handleSupprimerCompte}>
...
const EspaceClient = () => {
  ...
  const renderContent = () => {
    switch (activeTab) {
      case "mon-compte":
        return <MonCompte setActiveTab={setActiveTab} stats={stats} />;
      case "mes-commandes":
        return <MesCommandes commandes={commandes} loading={loading} />;
      case "mes-adresses":
        return <MesAdresses />;
      ...
      <div className="ec-container">
        ...
        <div className="ec-layout">
          <aside className="ec-sidebar">
            <nav>
              <button
                className={activeTab === "mon-compte" ? "active" : ""}
                onClick={() => setActiveTab("mon-compte")}
              >
                <span className="icon"></span> Mon compte
              </button>
              <button
                className={activeTab === "mes-commandes" ? "active" : ""}
                onClick={() => setActiveTab("mes-commandes")}
              >
                <span className="icon"></span> Mes commandes
              </button>
              <button
                className={activeTab === "mes-adresses" ? "active" : ""}
                onClick={() => setActiveTab("mes-adresses")}
              >
                <span className="icon"></span> Mes adresses
              </button>
              <button
                className={activeTab === "mes-informations" ? "active" : ""}
                onClick={() => setActiveTab("mes-informations")}
              >
          ...
          <section className="ec-content">{renderContent()}</section>
        </div>
      </div>
    </div>
  );
