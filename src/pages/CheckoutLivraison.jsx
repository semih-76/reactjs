...
const CARRIERS = [
    { id: "colissimo", label: "Colissimo", delay: "2 à 3 jours ouvrés", price: 4.9 },
    { id: "chronopost", label: "Chronopost", delay: "24h", price: 9.9 },
    { id: "mondial_relay", label: "Mondial Relay", delay: "3 à 5 jours ouvrés", price: 3.5 },
];
...
const CheckoutLivraison = () => {
    ...
    const [deliveryMode, setDeliveryMode] = useState(checkoutData.deliveryMode || "domicile");
    const [selectedAddressId, setSelectedAddressId] = useState(checkoutData.selectedAddressId || null);
    const [carrier, setCarrier] = useState(checkoutData.carrier || null);
    const [newAddress, setNewAddress] = useState(
        checkoutData.newAddress || { firstName: "", lastName: "", street: "", city: "", zip: "", country: "France" }
    );
    ...
    useEffect(() => {
        const fetchAddresses = async () => {
            try {
                ...
                if (def && !checkoutData.selectedAddressId) setSelectedAddressId(def.id);
            } catch {
    ...
    const shippingCost = deliveryMode === "magasin" ? 0 : (selectedCarrier?.price || 0);
                ...
                <div className="checkout-main">
                    ...
                    <section className="checkout-section">
                        ...
                        <div className="delivery-mode-grid">
                            {[
                                { value: "domicile", icon: "", label: "Livraison à domicile", sub: "Reçu chez vous" },
                                { value: "magasin", icon: "", label: "Retrait en boutique", sub: "15 rue du Thé, Paris 75001 — Gratuit" },
                            ].map((m) => (
                                <label key={m.value} className={`delivery-mode-card ${deliveryMode === m.value ? "active" : ""}`}>
                                    <input type="radio" name="deliveryMode" value={m.value}
                                ...
                                <>
                                            ...
                                            {addresses.map((addr) => (
                                                <label key={addr.id}
                                                    ...
                                                    <div className="address-radio-body">
                                                        ...
                                                        <span>{addr.zip} {addr.city}, {addr.country}</span>
                                                        {addr.isDefault && <span className="badge-default">Par défaut</span>}
                                                    </div>
                                                </label>
                                            ))}
                                            <button type="button" className="btn-add-new-addr"
                                    ...
                                    {(showNewAddressForm || addresses.length === 0) && (
                                        <div className="new-address-form">
                                            {addresses.length > 0 && (
                                                <button type="button" className="btn-back-addr"
                    ...
                    {deliveryMode === "domicile" && (
                        <section className="checkout-section">
                            ...
                            <div className="carrier-list">
                                {CARRIERS.map((c) => (
                                    <label key={c.id} className={`carrier-card ${carrier === c.id ? "active" : ""}`}>
                      ...
                      {c.price === 0 ? "Gratuit" : `${c.price.toFixed(2)} €`}
                            ...
                            {errors.carrier && <span className="field-error">{errors.carrier}</span>}
                        </section>
                    )}
                        ...
                        <section className="checkout-section">
                            <div className="store-pickup-info">
                                ...
                                <div>
                                    ...
                                    <p>Lun–Sam : 10h–19h</p>
                                    <p>Votre commande sera disponible sous <strong>24h</strong>. Vous recevrez un email de confirmation.</p>
                                </div>
                            </div>
                        </section>
                    )}
